...
using Bundle = Hl7.Fhir.Model.Bundle;
    ...
    public class FirelyService : IFhirService
    {
        ...
        #region Encounter
        public async Task<IList<Patient>> GetEncounterAsync()
        {
            var bundle = await _fhirClient.SearchAsync<Encounter>(pageSize: 50);
            var result = new List<Encounter>();
            while (bundle != null)
            {
                result.AddRange(bundle.Entry.Select(p => (Encounter)p.Resource).ToList());
                bundle = await _fhirClient.ContinueAsync(bundle);
            }
        ...
        {
            var bundle = await _fhirClient.SearchAsync<Encounter>(summary: SummaryType.Count);
            return bundle.Total ?? 0;
        }

        public async Task<IList<Patient>> SearchEncounter(Encounter Encounter)
        {
            string identifier = Encounter.Identifier[0].Value;
            Bundle bundle;

            if (!string.IsNullOrEmpty(identifier))
            {
                bundle = await _fhirClient.SearchByIdAsync<Encounter>(identifier);

                if (bundle != null)
                    return bundle.Entry.Select(p => (Encounter)p.Resource).ToList();
            }
